...
/*
    ...
    */
    ...
    {
        return new ParallelDeadlineGroup(new WaitCommand(time), new LowerArm(arm), new IntakeCargo(pow, arm)).andThen(new LiftArm(arm));
    }

    public Command deliver(double time, double pow, Arm arm)
    {
        return new ParallelRaceGroup(new WaitCommand(time), new DeliverCargo(pow, arm));
    }
