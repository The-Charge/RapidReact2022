...
public class AutoTele extends CommandBase {
    ...
    private static final double HANG_VOL_R = -0.45;
    private static final double HANG_VOL_L = -0.5;
    ...
    public void execute() {
        if (!isLeftStopped){
            if(m_telescope.isLeftStopped()){
                if (isPivotNext)leftSpeed = HANG_VOL_L;
                else leftSpeed = 0;
                isLeftStopped = true;
                m_telescope.runTele(leftSpeed,rightSpeed);
            }
        ...
        if (!isRightStopped){
            if(m_telescope.isRightStopped()){
                if (isPivotNext)rightSpeed = HANG_VOL_R;
                else rightSpeed = 0;
                isRightStopped = true;
                m_telescope.runTele(leftSpeed,rightSpeed);
            }
