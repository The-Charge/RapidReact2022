...
private WPI_TalonSRX rightTeleMotor;
	...
	private static final double PIVOT_kP = 0.33;
...
rightTeleMotor = new WPI_TalonSRX(8);
	...
	public void runTele(double l, double r) {
		if(Math.abs(l-leftSpeed) > TELE_DEADBAND){
			for(int i = 0; i < ENC_ARRAY_LEN; i++){
				leftEnc[i] = i*ENC_DIFFERENCE;
			}
		}
		leftSpeed = l;
		if(Math.abs(r-rightSpeed) > TELE_DEADBAND){
			for(int i = 0; i < ENC_ARRAY_LEN; i++){
				rightEnc[i] = i*ENC_DIFFERENCE;
			}
		}
		rightSpeed = r;
	...
	public void runLeftMotionMagic(double dist){
		SmartDashboard.putNumber("tele output", leftTeleMotor.getMotorOutputVoltage());
